...
// цифровые форматы можно переводить между друг другом без вызова метода структуры Convert. достаточно нариспть в скобках перед выражением псевдоним типа: (int), (decimal), (long), (sbyte). приведение ставится перед всем выражением, потому что имеет наивысший приоритет и все элементы будут приведены до вычисления. то есть нельзя ставить перед скобками, нужно ставить перед одним из элементов оператора непосредственно:
