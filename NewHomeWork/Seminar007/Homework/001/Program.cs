		...
		*/
		...
		double[,] GetArray(int rows, int columns)
		{
			...
			for (int i = 0; i < rows; i++)
			{
				...
				{
					// оператор, возвращающий 9/10 раз дробные числа: array[i, j] = Convert.ToDouble(new Random().Next(-99, 100)) / 10;

					// следующий оператор возвращает десятичные числа статистически 1/2 раз:
